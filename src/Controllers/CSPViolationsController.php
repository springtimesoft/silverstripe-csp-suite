...
class CSPViolationsController extends Controller
{
    ...
    public function index(HTTPRequest $request)
    {
        ...
        if (isset($json['csp-report'])) {
            // This report was sent as a result of the "report-uri" directive. (CSP Level 2, legacy)
            ...
            // This report was sent as a result of the "report-to" directive. (CSP Level 3+)
    ...
    protected function getOrCreateViolation(array $cspReport): CSPViolation
    {
        $violationData = [
            ...
            self::SOURCE_FILE         => $this->getDataForAttribute($cspReport, self::SOURCE_FILE),
        ];
    ...
    /**
     * Retrieve the user agent of the browser submitting the report, and apply it to the Violation.
     */
    protected function setUserAgent(CSPViolation $violation)
    {
        ...
        if (!$dbEntry) {
            // Prepare a human-readable label
            $parsedAgent = Parser::create()->parse($raw);

            $humanLabel  = sprintf(
                "%s (%s)",
                $parsedAgent->ua->toString(),
                $parsedAgent->os->toString(),
            );

            $dbEntry = CSPUserAgent::create(['Name' => $humanLabel, 'Raw' => $raw]);
            $dbEntry->write();
        }

        $violation->UserAgents()->add($dbEntry);
    }
    ...
    protected function getDataForAttribute(array $cspReport, string $attribute): mixed
    {
        ...
        if ('report-uri' === $cspReport[self::REPORT_DIRECTIVE]) {
            switch ($attribute) {
                ...
                // Source File is a CSP Level 3+ feature
        ...
        switch ($attribute) {
            ...
            case self::SOURCE_FILE:
                return $cspReport['sourceFile'] ?? null;
